...
5. `validate` → `render --preset youtube-16x9` → `package` (per-feature `enhance` flags drive overlays — do NOT pass `--enhance full`, it would also switch on particles). `package` renders `outputs/description.txt` from the template with chapter timestamps (from `chapters.json`) + recap + summary. Render Shorts ONLY if the hint asks (shorts/9x16/all): add `{preset: shorts-9x16}` to `outputs` then `render --all`.
