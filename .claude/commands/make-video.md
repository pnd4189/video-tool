...
1. Inspect the asset folder. Identify the voice file, image files, optional video clips (.mp4/.mov), the music FOLDER, and any intro thumbnail / ending image (AGENTS.md "Auto-detecting intro / ending images"). Use absolute paths.
2. Run `init-job` (point `--music` at the music folder), then patch `job.yaml`: `assets.policy: allow-missing-local`, `captions.mode: off`, and `inputs.intro_image` / `inputs.ending_image` when detected (Pitfalls #1, #3).
...
4. `validate` → `render --preset youtube-16x9` → `package`. Render Shorts ONLY if the hint asks (shorts/9x16/all): add `{preset: shorts-9x16}` to `outputs` then `render --all`.
5. Report the output mp4 paths, thumbnail path, and a one-line summary (durations, scene count, preset list, whether intro/ending applied). If staged from gdrive, report the `Output/` path + local space reclaimed. Quote any `ffprobe` codec check.
