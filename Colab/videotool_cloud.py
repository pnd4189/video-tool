...
# Job preparation lives in the installed package (shared with the local path). These wrappers keep
# the old API and import lazily: the notebooks import this module BEFORE `setup()` installs videotool.
...
def ensure_job_yaml(job_dir: Path) -> Path:
    """A hardened job.yaml (allow-missing-local, captions off, script set), created by init-job
    when the folder has none."""
    from videotool.creative.prepare import ensure_job_yaml as _ensure_job_yaml  # noqa: PLC0415

    return _ensure_job_yaml(job_dir)
...
def _run_cli(args: list[str]) -> None:
    ...
    from videotool.creative.prepare import run_cli  # noqa: PLC0415
